using System;
using System.Collections.Generic;
using System.Text;
using System.Data.Common;
using System.Data;
using System.Data.SqlServerCe;
using System.IO;
using DataHelper;

namespace CEDataHelper
{
    public class CEBaseDataHelper : IDisposable
    {
        #region Protected

        protected enum DataSourceType
        {
            SQLCeServer = 0
        }
        
        protected DataSourceType _DataSourceType;
        protected SqlCeConnection _Connection;

        protected SqlCeTransaction _Transaction;
        protected SqlCeDataAdapter _DataAdapter;
        protected SqlCeEngine _Engine;

        public CEBaseDataHelper()
        {
            this._Connection = this.GetConnection();
        }

        public void OpenDB()
        {
            if (this._Connection.State == ConnectionState.Closed)
                this._Connection.Open();
        }

        public void CloseDB()
        {
            if (this._Connection.State == ConnectionState.Open)
                this._Connection.Close();
        }

        #endregion

        public void BeginTransaction()
        {
            if (this._Transaction == null)
                this._Transaction = this._Connection.BeginTransaction();
        }

        public void CommitTransaction()
        {
            this._Transaction.Commit();
            this._Transaction = null;
        }

        public void RollBackTransaction()
        {
            this._Transaction.Rollback();
            this._Transaction = null;
        }

        public void BeginTransactionSnapshot()
        {
            if (this._Transaction == null)
            {
                this._Transaction = this._Connection.BeginTransaction(IsolationLevel.Snapshot);

            }
        }

        public void CommitTransactionSnapshot()
        {
            this._Transaction.Commit();
        }

        public void RollBackTransactionSnapshot()
        {
            this._Transaction.Rollback();
        }

        public void Dispose()
        {
            try
            {
                if (_Connection != null)
                {
                    if (_Connection.State != ConnectionState.Closed)
                        _Connection.Close();

                    _Connection.Dispose();
                }
            }
            catch
            {
                
            }
        }

        //public int ExecuteNonQuery(string dbStatement)
        //{
        //    SqlCommand Command = this._Connection.CreateCommand();
        //    Command.CommandTimeout = 0;
        //    Command.CommandText = dbStatement;
        //    Command.CommandType = CommandType.Text;

        //    return Command.ExecuteNonQuery();
        //}

        public int ExecuteNonQuery(string dbStatement)
        {
            return this.ExecuteNonQuery(dbStatement, null);
        }

        public int ExecuteNonQuery(string dbStatement, params object[] paramValues)
        {
            string[] rawParameters = dbStatement.Split('@');
            CEParameterInfo[] dbParameters = new CEParameterInfo[paramValues.Length];

            int index = -1;
            foreach (string rawParameter in rawParameters)
            {
                if (index > -1)
                {
                    dbParameters[index] = new CEParameterInfo(rawParameter.Substring(0, rawParameter.IndexOf(' ') == -1 ? rawParameter.Length : rawParameter.IndexOf(' ')), paramValues[index]);

                    if (index == paramValues.Length)
                        break;
                }
                index++;
            }

            return this.ExecuteNonQuery(dbStatement, dbParameters);
        }

        public int ExecuteNonQuery(string dbStatement, params CEParameterInfo[] dbParameters)
        {
            SqlCeCommand Command = this._Connection.CreateCommand();
            Command.CommandTimeout = 0;
            Command.CommandText = dbStatement;
            Command.CommandType = CommandType.Text;

            if (this._Transaction != null)
                Command.Transaction = this._Transaction;

            if (dbParameters != null)
                foreach (CEParameterInfo dbParameterInfo in dbParameters)
                {
                    SqlCeParameter dbParameter = new SqlCeParameter(dbParameterInfo.Name, dbParameterInfo.Type, dbParameterInfo.Size);
                    dbParameter.Value = dbParameterInfo.Value;

                    Command.Parameters.Add(dbParameter);
                }

            return Command.ExecuteNonQuery();
        }

        public SqlCeDataReader ExecuteReader(string dbStatement)
        {
            SqlCeCommand Command = this._Connection.CreateCommand();
            Command.CommandTimeout = 0;
            Command.CommandText = dbStatement;
            Command.CommandType = CommandType.Text;

            SqlCeDataReader DataReader;
            DataReader = Command.ExecuteReader();
            //DataReader.Read();

            return DataReader;
        }

        public DataTable ExecuteDataTable(string dbStatement)
        {
            DataTable dt = new DataTable();
            
            SqlCeCommand Command = this._Connection.CreateCommand();
            Command.CommandTimeout = 0;
            Command.CommandText = dbStatement;
            Command.CommandType = CommandType.Text;

            _DataAdapter = new SqlCeDataAdapter();            
            _DataAdapter.SelectCommand = Command;
            _DataAdapter.Fill(dt);            
            _DataAdapter.Dispose();
            return dt;
        }

        public DataSet ExecuteDataSet(string dbStatement)
        {
            return this.ExecuteDataSet(dbStatement, null);
        }

        public DataSet ExecuteDataSet(string dbStatement, params object[] paramValues)
        {
            string[] rawParameters = dbStatement.Split('@');
            CEParameterInfo[] dbParameters = new CEParameterInfo[paramValues.Length];

            int index = -1;
            foreach (string rawParameter in rawParameters)
            {
                if (index > -1)
                {
                    dbParameters[index] = new CEParameterInfo(rawParameter.Substring(0, rawParameter.IndexOf(' ') == -1 ? rawParameter.Length : rawParameter.IndexOf(' ')), paramValues[index]);

                    if (index == paramValues.Length)
                        break;
                }
                index++;
            }

            return this.ExecuteDataSet(dbStatement, dbParameters);
        }

        public DataSet ExecuteDataSet(string dbStatement, params CEParameterInfo[] dbParameters)
        {
            DataSet value = new DataSet();

            SqlCeCommand Command = this._Connection.CreateCommand();
            Command.CommandTimeout = 0;
            Command.CommandText = dbStatement;
            Command.CommandType = CommandType.Text;

            if (this._Transaction != null)
                Command.Transaction = this._Transaction;


            if (dbParameters != null)
                foreach (CEParameterInfo dbParameterInfo in dbParameters)
                {
                    SqlCeParameter dbParameter = Command.CreateParameter();
                    dbParameter.ParameterName = dbParameterInfo.Name;
                    dbParameter.Value = dbParameterInfo.Value;
                    dbParameter.SqlDbType = dbParameterInfo.Type;
                    dbParameter.Size = dbParameterInfo.Size;

                    Command.Parameters.Add(dbParameter);
                }

            _DataAdapter = new SqlCeDataAdapter();
            _DataAdapter.SelectCommand = Command;
            _DataAdapter.Fill(value);

            foreach (DataTable table in value.Tables)
            {
                foreach (DataRow row in table.Rows)
                {
                    for (int i = 0; i < table.Columns.Count; i++)
                    {
                        if (table.Columns[i].DataType == Type.GetType("System.String", true))
                            row[i] = row[i].ToString().TrimEnd();
                    }
                }
            }
            _DataAdapter.Dispose();
            return value;
        }

        /// <summary>
        /// Creates a HandyDB.sdf in the handy device.
        /// </summary>
        /// <returns></returns>
        public bool DBCreate()
        {
            string connectionString = HandyConfiguration.Settings["HandyDB"];
            //string ConnectionString = "Data Source='\\Temp\\HandyDB.sdf" + name + "'; LCID=1033; Password=systemadmin; Encrypt=true";

            if (!File.Exists("\\Temp\\HandyDB.sdf"))
            {
                try
                {
                    _Engine = new SqlCeEngine(connectionString);
                    _Engine.CreateDatabase();
                }
                catch
                {
                    return false;
                }
                finally
                {
                    _Engine.Dispose();
                    
                }
                return true;
            }
            else
                return false;

        }

        #region private

        private SqlCeConnection GetConnection()
        {
            string connectionString = HandyConfiguration.Settings["HandyDB"];
            //string connectionString = "Data Source='\\HandyDB.sdf'; LCID=1033; Password=systemadmin; Encrypt=true";
            return new SqlCeConnection(connectionString);
        }

        private SqlCeConnection GetLetConnection()
        {
            string connectionString = HandyConfiguration.Settings["LetTestDB"];

            return new SqlCeConnection(connectionString);
        }

        #endregion //private
    }
}
