...
namespace FTPClient
{
    ...
    public class FTPConnection
    {
        ...
        public virtual void Open(string remoteHost, int remotePort, string user, string password, FTPMode pMode)
        {
            ...
            //CREATE DIRECTORY
            tempMessageList = SendCommand("MKD " + "HANDY_" + Handy.Lib.CommonFunctions.HandyNo);
            returnValue = GetMessageReturnValue((string)tempMessageList[0]);
            string retstr = tempMessageList[0].ToString();
            if (returnValue != 257 && !retstr.Replace("550", "").TrimStart().EndsWith("Already exists, failed to create."))
            {
                Close();
                throw new Exception((string)tempMessageList[0]);
            }
            ...
            //CREATE DIRECTORY
            tempMessageList = SendCommand("MKD " + "HANDY_" + Handy.Lib.CommonFunctions.HandyNo + @"\" + Handy.Lib.CommonFunctions.HandyFTPSeqNo);
            returnValue = GetMessageReturnValue((string)tempMessageList[0]);
            retstr = tempMessageList[0].ToString();
            if (returnValue != 257 && !retstr.Replace("550", "").TrimStart().EndsWith("Already exists, failed to create."))
            {
                Close();
                throw new Exception((string)tempMessageList[0]);
            ...
            tempMessageList = SendCommand("MKD " + "HANDY_" + Handy.Lib.CommonFunctions.HandyNo + @"\" + Handy.Lib.CommonFunctions.HandyFTPSeqNo + @"\Archives");
            returnValue = GetMessageReturnValue((string)tempMessageList[0]);
            retstr = tempMessageList[0].ToString();
            if (returnValue != 257 && !retstr.Replace("550", "").TrimStart().EndsWith("Already exists, failed to create."))
            {
